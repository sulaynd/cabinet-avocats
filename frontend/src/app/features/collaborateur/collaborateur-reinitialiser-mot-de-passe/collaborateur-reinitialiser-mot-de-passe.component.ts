...
@Component({
  selector: 'app-collaborateur-reinitialiser-mot-de-passe',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, RouterLink, MatFormFieldModule, MatInputModule, MatButtonModule],
  templateUrl: './collaborateur-reinitialiser-mot-de-passe.component.html',
})
export class CollaborateurReinitialiserMotDePasseComponent implements OnInit {
  chargement = false;
  reussi = false;
  erreur = '';
  lienInvalide = false;
  ...
  constructor(private collaborateurAuth: CollaborateurAuthService, private route: ActivatedRoute, private router: Router) {}
  ...
  soumettre(): void {
    ...
    this.collaborateurAuth.reinitialiserMotDePasse(this.email, this.token, this.form.value.password!).subscribe({
      next: () => {
