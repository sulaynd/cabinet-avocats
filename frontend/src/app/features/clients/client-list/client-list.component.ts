...
export class ClientListComponent implements OnInit, AfterViewInit {
  ...
  readonly colonnes = ['nom', 'type', 'email', 'telephone', 'ville', 'created_at', 'actions'];
  ...
  /**
   * Recherche "en direct" comme dans le mockup (filtre à chaque frappe, pas
   * besoin d'Entrée) — avec un léger délai pour ne pas solliciter le serveur
   * à chaque lettre tapée (le mockup filtrait en mémoire, ici on interroge
   * l'API à chaque changement, donc le debounce évite le flot de requêtes).
   */
  ...
  ngAfterViewInit(): void {
    ...
    this.dataSource.sortingDataAccessor = (client, colonne) => {
      if (colonne === 'nom') return this.nomAffiche(client).toLowerCase();
      return (client as any)[colonne] ?? '';
    };
  }
